...
public class JdbcTemplateTaskRepository implements TaskRepository{
    ...
    public Task findTaskById(Long taskId) {
        ...
        return result.stream().findAny().orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Does not exist task_id = " + taskId));
    }

    @Override
    public List<TaskResponseDto> findAllTasks() {
        return jdbcTemplate.query(SELECT_PREFIX + ";", taskResponseDtoRowMapper());
    }

    @Override
    public List<TaskResponseDto> findTasksByEmail(String email) {
        String query = SELECT_PREFIX + " WHERE u.email = ?;";
        return jdbcTemplate.query(query, taskResponseDtoRowMapper(), email);
    }

    @Override
    public List<TaskResponseDto> findTasksByDate(LocalDate date) {
        String query = SELECT_PREFIX + " WHERE DATE(t.updated_date_time) = ? ORDER BY t.updated_date_time DESC;";
    ...
    public List<TaskResponseDto> findTasksByEmailAndDate(String email, LocalDate date) {
        String query = SELECT_PREFIX + " WHERE u.email = ? AND DATE(t.updated_date_time) = ? ORDER BY t.updated_date_time DESC;";
